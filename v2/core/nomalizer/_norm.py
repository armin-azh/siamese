...
class MaskNormalizer(HeadPoseEstimatorNormalizer):
    def __init__(self, name=None, *args, **kwargs):
        super(MaskNormalizer, self).__init__(name=name, *args, **kwargs)

    def normalize_output(self, mat: np.ndarray, tilt_norm: Tuple[float, float], pan_norm: Tuple[float, float],
                         rescale: float) -> np.ndarray:
        raise DisableMethodWarning("this method is disable for MaskNormalize class")
    ...
    def normalize(self, mat: np.ndarray, **kwargs) -> np.ndarray:
        ...
        if len(mat.shape) == 3:
            ...
            raise InCompatibleDimError("input image should be in 3 dim")


class Face160Cropper(BaseNormalizer):
    def __init__(self, name=None, *args, **kwargs):
        self._box_tensor_shape = tf.TensorShape([None, 4])
        super(Face160Cropper, self).__init__(name=name, *args, **kwargs)
        ...
        if len(mat.shape) == 3:
            ...
            for box in b_mat:
                ...
                if _x_min >= 0 and _y_min >= 0 and _x_max < _ori_width and _y_max < _ori_height:
                    ...
                    if cropping == 'small' or cropping == 'large':
                        c_pic = cv2.resize(c_pic,
                                           (int(160 * (1 + 2 * offset_per)),
                                            int(160 * (1 + 2 * offset_per))),
                                           interpolation=interpolation)
                    else:
                        if c_pic.shape[0] > c_pic.shape[1]:
                            c_pic = cv2.resize(c_pic,
                                               (int((160 * c_pic.shape[0] / c_pic.shape[1]) * (
                                                       1 + 2 * offset_per)),
                                                int(160 * (1 + 2 * offset_per))),
                                               interpolation=interpolation)
                        else:
                            c_pic = cv2.resize(c_pic, (int(160 * (1 + 2 * offset_per)),
                                                       int((160 * c_pic.shape[1] / c_pic.shape[0]) * (
                                                               1 + 2 * offset_per))), interpolation=interpolation)
                    _cropped.append(c_pic)
                else:
                    _cropped.append(np.empty(0))
